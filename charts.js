...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var graphValues = sampleValues.slice(0,10).reverse();
    ...
    var metadataFiltered = data.metadata.filter(md => md.id == sample)[0];

    // 3. Create a variable that converts the washing frequency to a floating point number.
    var wfreq = parseFloat(metadataFiltered.wfreq);
    ...
    var gaugeData = [{
      ...
      gauge: {
              axis: {range: [null,10], showticklabels: true,},
              steps: [{range:[8,10], color:'green'},
